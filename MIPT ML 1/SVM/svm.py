...
def rbf(x_1, x_2, sigma=1.):
    ...
    '''
    distances = torch.exp(-((x_2 ** 2).sum(axis=1) + (x_1[:,:,None] ** 2).sum(axis=1) - 2 * np.dot(x_1, x_2.T))/2/(sigma**2))
    ...
    def fit(self, X, Y):
        ...
        for epoch in range(self.epochs):
            ...
            for i in range(0, n_obj, self.batch_size):
                ...
                preds = torch.matmul(k_batch, self.betas) + self.bias # get the matrix product using SVM parameters: self.betas and self.bias
                preds = preds.flatten()
